...
def create_heatmap(
    ...
    if rate_type == "energy":
        ...
        df = tariff_viewer.demand_weekday_df if is_weekday else tariff_viewer.demand_weekend_df
        day_type = "Weekday" if is_weekday else "Weekend"
...
def create_flat_demand_chart(tariff_viewer: TariffViewer, dark_mode: bool = False) -> go.Figure:
    """
...
def create_load_duration_curve(load_profile_df: pd.DataFrame, dark_mode: bool = False) -> go.Figure:
    """
    ...
    sorted_loads = load_profile_df['load_kW'].sort_values(ascending=False).reset_index(drop=True)
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(
            color='#1e40af' if not dark_mode else '#60a5fa',
            width=3
        ),
        hovertemplate=(
...
def display_rate_statistics(tariff_viewer: TariffViewer) -> None:
    ...
    if all_energy_rates:
        ...
        with col3:
            st.metric("Average Rate", f"${sum(all_energy_rates)/len(all_energy_rates):.4f}/kWh")
        with col4:
            st.metric("Rate Spread", f"${max(all_energy_rates) - min(all_energy_rates):.4f}/kWh")
    ...
    if all_demand_rates:
        ...
        with col3:
            st.metric("Average Rate", f"${sum(all_demand_rates)/len(all_demand_rates):.4f}/kW")
        with col4:
            st.metric("Rate Spread", f"${max(all_demand_rates) - min(all_demand_rates):.4f}/kW")
