...
def _render_openei_import_section() -> None:
    ...
    if configured_api_key:
        ...
        st.sidebar.info("💡 **API Key**: Add to secrets.toml, set environment variable, or enter below")
        api_key_input = st.sidebar.text_input(
            "OpenEI API Key:",
            type="password",
    ...
    # Show link to OpenEI
    st.sidebar.caption("[🔗 Browse tariffs on OpenEI.org](https://openei.org/wiki/Utility_Rate_Database)")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if items:
                    ...
                    st.sidebar.info("🔄 Refresh the page or reselect to see the new tariff in the dropdown.")
                ...
                st.sidebar.error("❌ Invalid API key. Please check your OpenEI API key.")
            ...
            st.sidebar.error("❌ Connection error. Please check your internet connection.")
        except Exception as e:
