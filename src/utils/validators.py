    ...
    try:
        ...
        for field in required_fields:
            if field not in tariff or not tariff[field]:
                validation_results['errors'].append(f"Missing or empty required field: {field}")
                validation_results['is_valid'] = False
    ...
    for i, rate_period in enumerate(energy_rates):
        ...
        try:
            ...
            if rate_value > 1:  # Assuming rates over $1/kWh are unusual
                result['warnings'].append(f"Energy rate period {i} has unusually high rate: ${rate_value:.4f}/kWh")
    ...
    for i, rate_period in enumerate(demand_rates):
        ...
        try:
            ...
            if rate_value > 100:  # Assuming rates over $100/kW are unusual
                result['warnings'].append(f"Demand rate period {i} has unusually high rate: ${rate_value:.2f}/kW")
        except (ValueError, TypeError):
...
def _validate_schedules(tariff: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if weekday_schedule:
        if len(weekday_schedule) != 12:
            result['errors'].append(f"Energy weekday schedule should have 12 months, found {len(weekday_schedule)}")
            result['is_valid'] = False
        else:
            for month_idx, month_schedule in enumerate(weekday_schedule):
                if not isinstance(month_schedule, list):
                    result['errors'].append(f"Energy weekday schedule month {month_idx + 1} should be a list")
                    result['is_valid'] = False
                elif len(month_schedule) != 24:
                    result['errors'].append(f"Energy weekday schedule month {month_idx + 1} should have 24 hours, found {len(month_schedule)}")
                    result['is_valid'] = False
    
    # Validate weekend schedule
    if weekend_schedule:
        if len(weekend_schedule) != 12:
            result['errors'].append(f"Energy weekend schedule should have 12 months, found {len(weekend_schedule)}")
            result['is_valid'] = False
        else:
            for month_idx, month_schedule in enumerate(weekend_schedule):
                if not isinstance(month_schedule, list):
                    result['errors'].append(f"Energy weekend schedule month {month_idx + 1} should be a list")
                    result['is_valid'] = False
                elif len(month_schedule) != 24:
                    result['errors'].append(f"Energy weekend schedule month {month_idx + 1} should have 24 hours, found {len(month_schedule)}")
                    result['is_valid'] = False
    ...
    try:
        ...
        if not pd.api.types.is_numeric_dtype(df['load_kW']):
            ...
            if (df['load_kW'] < 0).any():
                ...
                validation_results['warnings'].append(f"Found {negative_count} negative load values")
            
            # Check for missing values
            if df['load_kW'].isna().any():
                missing_count = df['load_kW'].isna().sum()
                validation_results['warnings'].append(f"Found {missing_count} missing load values")
            
            # Check for unrealistic values
            max_load = df['load_kW'].max()
            if max_load > 100000:  # 100 MW seems like a reasonable upper bound
                validation_results['warnings'].append(f"Very high peak load detected: {max_load:.1f} kW")
