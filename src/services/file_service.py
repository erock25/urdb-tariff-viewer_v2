...
class FileService:
    ...
    def load_csv_file(file_path: Union[str, Path]) -> pd.DataFrame:
        ...
        try:
            ...
            st.error(f"Error loading CSV file {file_path}: {str(e)}")
            raise
    ...
    def save_csv_file(df: pd.DataFrame, file_path: Union[str, Path]) -> None:
        ...
        try:
            ...
            st.error(f"Error saving CSV file {file_path}: {str(e)}")
            raise
    
    @staticmethod
    def create_modified_filename(original_path: Union[str, Path], prefix: str = "modified_") -> Path:
        """
    ...
    @staticmethod
    def validate_file_size(file_path: Union[str, Path], max_size_mb: Optional[float] = None) -> bool:
        """
