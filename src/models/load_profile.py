...
from src.config.constants import (
    ...
    DEFAULT_DAILY_VARIATION, DEFAULT_NOISE_LEVEL, DEFAULT_TOU_PERCENTAGES
)
...
class LoadProfileGenerator:
    ...
    def __init__(self, tariff: Dict, avg_load: float, load_factor: float = DEFAULT_LOAD_FACTOR, year: int = 2025):
        """
    ...
    def validate_profile(self, profile_df: pd.DataFrame, tolerance: float = 0.05) -> Dict[str, bool]:
        """
        ...
        load_factor_error = abs(stats['load_factor'] - self.load_factor) / self.load_factor
        
        return {
            'avg_load_valid': avg_error <= tolerance,
            'load_factor_valid': load_factor_error <= tolerance,
            'no_negative_values': stats['min_kw'] >= 0,
            'reasonable_peak': stats['peak_kw'] <= self.avg_load * 5  # Peak shouldn't be more than 5x average
        }
