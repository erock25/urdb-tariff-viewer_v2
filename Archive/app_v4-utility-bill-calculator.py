            ...
            if st.button("🧮 Calculate Utility Costs", type="primary"):
                try:
                    with st.spinner("Calculating utility costs..."):
                        ...
                        peak_monthly_cost = results['total_charge'].max()
                        
                        col1, col2, col3, col4 = st.columns(4)
                        with col1:
                            st.metric("Annual Total Cost", f"${total_annual_cost:,.2f}")
                        with col2:
                            st.metric("Annual Total kWh", f"{total_annual_kwh:,.0f}")
                        with col3:
                            st.metric("Average Monthly Cost", f"${avg_monthly_cost:,.2f}")
                        with col4:
                            st.metric("Peak Monthly Cost", f"${peak_monthly_cost:,.2f}")
